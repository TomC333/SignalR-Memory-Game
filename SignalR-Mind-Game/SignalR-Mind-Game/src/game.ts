...
const activePlayerLabel: HTMLElement = document.getElementById("active-player-label");
const gameWindow: HTMLElement = document.getElementById("game");
const gameNotFoundInput: HTMLInputElement = document.querySelector("#game-not-created-input-container input");
const gameFinishedInput: HTMLInputElement = document.querySelector("#game-finished-input-container input");

const connection = new signalR.HubConnectionBuilder()
    .withUrl("/game")
    .build();

connection.on("CurrentPlayerIs", (isCurrentUsersTurn: boolean) => {
...
connection.on("UserPlayed", (catId: number) => {
...
const gameManager: GameManager = new GameManager(connection, gameId);
const loader = new Loader();


SpriteManager.loadInitialResources((progress: number) => {
